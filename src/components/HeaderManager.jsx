...
export default function HeaderManager({ isOpen, onClose, config, updateConfig }) {
                ...
                <div className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="เช่น โรงเรียนวังเด็กพัฒนา"
                        />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">ภาคเรียนที่</label>
                            <input 
                                type="text" 
                                value={config.term || ''}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">ปีการศึกษา</label>
                            <input 
                                type="text" 
                    ...
                    {/* 🌟 ส่วนแก้ไขข้อความ ตารางเรียน / ตารางสอน แบบ 100% */}
                    <div className="mt-4 pt-4 border-t border-slate-100 space-y-4">
                        <h3 className="font-bold text-sm text-indigo-700">📝 ปรับแต่งข้อความซ้าย-ขวา</h3>
                        <div>
                            <label className="block text-xs font-bold text-slate-500 mb-1">ข้อความฝั่งซ้าย (ก่อนปีการศึกษา)</label>
                            <input 
                                type="text" 
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">ข้อความขวา (โหมดครู)</label>
                                <input 
                                    type="text" 
                                    value={config.textRightTeacher || 'ตารางสอน คุณครู'}
                                    onChange={(e) => updateConfig('textRightTeacher', e.target.value)}
                                    className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">ข้อความขวา (โหมดนักเรียน)</label>
                                <input 
                                    type="text" 
                                    value={config.textRightStudent || 'ตารางเรียน ระดับชั้น'}
                                    onChange={(e) => updateConfig('textRightStudent', e.target.value)}
                                    className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                                />
                            </div>
                        </div>
